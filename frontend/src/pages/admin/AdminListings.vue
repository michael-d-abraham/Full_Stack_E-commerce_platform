  ...
  <div class="admin-page admin-listings">
    ...
    <PageReveal :ready="!loading">
      ...
      <p v-else-if="!items.length" class="admin-float admin-float--padded admin-page-empty">
        No wanna do's yet.
        <router-link to="/admin/new">Add your first wanna do</router-link>
      </p>
      ...
      <ul class="admin-mobile-cards" aria-label="Wanna Do's">
...
async function onDelete(p) {
  if (!window.confirm(`Delete "${p.title}"? It will be hidden from the Wanna Do's page.`)) return;
  try {
