  ...
  <div class="admin-page">
    <header class="admin-page-header">
      ...
      <div class="admin-page-header__end">
        <ClerkUserButton v-if="clerkEnabled" />
      </div>
    </header>
    ...
    <section class="admin-float admin-float--padded admin-settings-section">
      ...
      <p class="admin-settings-section__copy">
        <template v-if="clerkEnabled">Signed in with Clerk. <span v-if="displayName">{{ displayName }}</span></template>
        <template v-else>Signed in with a local admin session (Clerk not configured).</template>
      </p>
    ...
    <section v-if="clerkEnabled" class="admin-float admin-float--padded admin-settings-section">
...
const ClerkUserButton = clerkEnabled
  ? defineAsyncComponent(() => import('@clerk/vue').then((m) => m.UserButton))
  : null;
...
const displayName = computed(() => {
  ...
  return sessionUser.value.username || sessionUser.value.email || '';
});
...
onMounted(async () => {
  try {
    sessionUser.value = await getAdminSession();
  } catch {
