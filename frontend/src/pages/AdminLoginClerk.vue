<template>
  <div class="admin-auth">
    ...
    <div class="admin-auth__panel">
      ...
      <!-- Sign in -->
      <template v-if="step === 'sign-in' && isReady && isSignedIn">
        <p class="admin-auth__notice" role="status">You're signed in.</p>
        <button
          type="button"
...
import { resolveAdminRedirectTarget } from '../services/clerkToken.js';
...
const redirectTarget = computed(() => resolveAdminRedirectTarget(route.query.redirect));
...
watch(
  ...
  ([loaded, signedIn]) => {
    if (loaded && signedIn && step.value === 'sign-in' && !busy.value) {
      enterDashboard();
    }
  },
  { immediate: true }
