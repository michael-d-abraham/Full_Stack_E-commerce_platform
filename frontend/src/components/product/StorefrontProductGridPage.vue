<template>
  <div class="gallery-page">
    <header class="gallery-header">
      <h1 class="page-hero-title gallery-header__title">{{ pageTitle }}</h1>
    </header>

    <p v-if="loading" class="gallery-status">Loading…</p>
    <p v-else-if="error" class="error">{{ error }}</p>
    <p v-else-if="!products.length" class="gallery-status">{{ emptyMessage }}</p>

    <template v-else>
      <section class="gallery-section" :aria-label="sectionLabel">
...
import GalleryProductCard from './GalleryProductCard.vue';
import ProductDetailOverlay from './ProductDetailOverlay.vue';
...
function closeProduct() {
  ...
  router.replace({ name: props.routeName });
}
