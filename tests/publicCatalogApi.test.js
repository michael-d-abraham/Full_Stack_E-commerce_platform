...
describe('public catalog API', () => {
    ...
    it('returns slim list items with primary image only', async () => {
        ...
        expect(item.size_label).toBe('16x20');
    ...
    it('returns full public detail by slug without Stripe internals', async () => {
        ...
        expect(res.body.format).toBe('Giclée');
        expect(res.body.year_created).toBe(2024);
        expect(res.body.size_label).toBe('16x20');
        expect(res.body.product_images).toHaveLength(2);
        expect(res.body.product_images.map((img) => img.image_url)).toEqual([
            'https://ik.imagekit.io/demo/primary.jpg',
            'https://ik.imagekit.io/demo/secondary.jpg'
        ]);
